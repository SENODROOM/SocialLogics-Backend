const { SearchHistory, Trending } = require('../models');
const { PLATFORMS, getPlatformById, getPlatformsByCategory } = require('../utils/platforms');
...
const enhanceQuery = (q) => {
  const expansions = {
    ...
    'yt': 'youtube tutorial', 'diy': 'do it yourself',
    'irl': 'in real life', 'asmr': 'asmr',
  };
...
const calcTrendingScore = (count, dailyCount, lastSearched) => {
  ...
  const recencyBoost = Math.max(0, 1 - hoursSince / 72); // decay over 72h
...
const updateTrending = async (query, platform) => {
  ...
  try {
    ...
    if (existing) {
      ...
      existing.score = calcTrendingScore(existing.count, existing.dailyCount, existing.lastSearched);
      ...
      await Trending.create({
        ...
        count: 1, dailyCount: 1, weeklyCount: 1, score: 1,
        platforms: platform ? [platform] : [],
        lastSearched: new Date(),
      });
    }
    // Bust suggestions cache
...
exports.getPlatforms = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: { platforms, categories: require('../utils/platforms').PLATFORM_CATEGORIES } });
});

// ─── POST /api/search ────────────────────────────────────────────────────────
exports.search = asyncHandler(async (req, res) => {
  const {
    query, platform = 'all', category = '',
    contentType = 'all', filters = {}, sessionId = '',
    openAll = false,
  } = req.body;
  ...
  // Log history (async, non-blocking)
  if (req.user) {
    SearchHistory.create({
      user: req.user._id,
      query: q,
      normalizedQuery: normalizeQuery(q),
      platform, category, contentType, filters,
  ...
  res.json({
    ...
    data: {
      ...
      related,
      searchId: Date.now().toString(36)+Math.random().toString(36).substr(2),
      timestamp: new Date(),
    },
  });
});
...
exports.getSuggestions = asyncHandler(async (req, res) => {
  ...
  const [trending, history] = await Promise.all([
    Trending.find({ query: { $regex: '^' + normalizeQuery(q), $options: 'i' } })
      .sort({ score: -1 }).limit(8).select('displayQuery query score count'),
    req.user
      ? SearchHistory.find({
          user: req.user._id,
          normalizedQuery: { $regex: normalizeQuery(q), $options: 'i' },
        }).sort({ createdAt: -1 }).limit(5).select('query platform')
      : Promise.resolve([]),
  ]);

  const suggestions = [
    ...history.map(h => ({ query: h.query, platform: h.platform, type: 'history', icon: '↩' })),
    ...trending.map(t => ({
...
exports.getTrending = asyncHandler(async (req, res) => {
  ...
  const sortField = period === 'daily' ? 'dailyCount' : period === 'weekly' ? 'weeklyCount' : 'score';
  ...
  const trending = await Trending.find(filter)
    ...
    .select('displayQuery query count dailyCount weeklyCount score category lastSearched');
...
exports.recordClick = asyncHandler(async (req, res) => {
  ...
  if (req.user && query) {
    await SearchHistory.findOneAndUpdate(
      { user: req.user._id, query, createdAt: { $gte: new Date(Date.now() - 60000) } },
...
exports.getSearchStats = asyncHandler(async (req, res) => {
  ...
  const stats = { totalSearches, uniqueQueries, platforms: PLATFORMS.length, topQuery: topPlatform?.displayQuery };
